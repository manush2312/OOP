...
enum BankError{    // error for different account operations
	NO_ACCOUNT=-10,
	ACCOUNT_EXISTS,
	ACCOUNT_IN_USE,
...
int main(int argc, char **argv)
{
    ...
    // basically initialising mutexes
	for (int i = 0; i < MAX_CUSTOMERS; i++)
	{
		if (pthread_mutex_init(&customerMutex[i], NULL) != 0)    //pthread_mutex_init --> used for initialization
		{
	...
	if (pthread_attr_init(&kernelAttribute) != 0)   //Initialize thread attribute
	{
		abnormal_exit("pthread_attr_init()");
	}
	else if (pthread_attr_setscope(&kernelAttribute, PTHREAD_SCOPE_SYSTEM) != 0)  // Set thread attribute scope
	{
		abnormal_exit("pthread_attr_setscope()");
	}
	else if (sem_init(&actionLock, 0, 0) != 0)  // Initialize semaphore for action lock
	{
		abnormal_exit("sem_init()");
	}
	else if (pthread_mutex_init(&bankMutex, NULL) != 0)   //Initialize mutex for bank operations
	{
		abnormal_exit("pthread_mutex_init()");
	}
	else if (pthread_create(&tid, &kernelAttribute, session_acceptor_thread, 0) != 0)   // Create session acceptor thread
	{
		abnormal_exit("pthread_create()");
	}
	else if (pthread_create(&tid, &kernelAttribute, periodic_action_cycle_thread, 0) != 0)   // Create periodic action cycle thread
	{
		abnormal_exit("pthread_create()");
	}
	else
...
void abnormal_exit(const char* msg){    // throws error when something goes wrong
	perror(msg);
	exit(EXIT_FAILURE);
}

void set_iaddr(struct sockaddr_in *sockaddr, long x, unsigned int port)   // Initializes a sockaddr_in structure with the specified IP address (x) and port.
{
...
// this thread function creates a timer to trigger a periodic action (every 20 seconds) 
// that involves printing information about the active connections and customer accounts in the bank. 
// The thread continuously waits for the timer signal, processes the information, and then yields the CPU to other threads.

void *periodic_action_cycle_thread(void *ignore)   //executes a specific action at regular intervals.
{
	/* Variables declaration, index, sigaction struct and timer */
	int i;
	struct sigaction action;   //sigaction structure specifies how to handle a signal.
	struct itimerval interval;   // basically timer
	...
	{
        // in this loop we are waiting for the signal and printing active accounts and details..
		...
		sched_yield();    // allows other threads to run
	}
	return 0;
}

float parse_float(const char* string){       //Converts a string to a float, ensuring two decimal places precision
...
void server_response(int sock, char* response)    //Sends a response to the client
{
...
//Handles the client's session, interpreting commands and responding accordingly. Uses a loop to read commands from the 
// client and calls the appropriate functions to process them.
...
//This function is responsible for accepting incoming client connections and spawning a new thread to handle each client session.
